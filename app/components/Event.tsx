...
const EventSection: React.FC<EventSectionProps> = ({ darkMode, language }) => {
  ...
  return (
    <section
      id="event"
      className={`min-h-screen py-20 px-4 sm:px-6 md:px-8 lg:px-12 ${ 
    ...
    >
      ...
      >
        ...
        {/* Adicionado um grid responsivo para os eventos para melhor layout em telas maiores */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {events.map((event, index) => (
            <motion.div
              ...
              whileHover="hover" 
              transition={{ delay: index * 0.1 }} // Atraso sequencial para cada card de evento
            >
              {event.image && (
                <img
                  src={event.image}
                  alt={event.title}
                  className="w-full object-cover rounded-md mb-4 shadow-sm"
                  style={{ maxHeight: '200px', width: 'auto', margin: '0 auto' }} // Altura máxima e centralização para imagens
                />
              )}
              <div className="flex-grow">
                ...
                <div className="flex flex-wrap gap-3 mt-4 justify-end"> {/* Links alinhados à direita */}
                  ...
                  {(event.zoomMeetingId || event.zoomPasscode) && (
                    <div className={`flex items-center space-x-2 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'} mt-2 w-full justify-end`}> {/* w-full justify-end para alinhar */}
                        {event.zoomMeetingId && <span className="whitespace-nowrap">ID: {event.zoomMeetingId}</span>}
                        {event.zoomMeetingId && event.zoomPasscode && <span className="whitespace-nowrap">&bull;</span>}
